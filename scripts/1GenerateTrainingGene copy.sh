...
### working directory, where the output will be generated
...
## Same but for weaker hints alignment, only filter here is start + stop, and of course not the highQ ones
grep '>' ${TRANSLATION_OUTPUT}/*.cds.all.fa | \
...
cd-hit -i ${workingdir}/TrainingGene/Workingdir/tmp/transcriptome.pep.highQ.fa -o ${workingdir}/TrainingGene/Workingdir/tmp/transcriptome.pep.highQ.representative.fa -c 0.8 -aS 0.9 -g 1 -d 0 -n 3 -M 75000 -T 32
...
cd-hit -i ${workingdir}/Hints/Workingdir/tmp/transcriptome.pep.weakerQ.fa -o ${workingdir}/Hints/Workingdir/tmp/transcriptome.pep.weakerQ.representative.fa -c 0.8 -aS 0.9 -g 1 -d 0 -n 3 -M 75000 -T 32
